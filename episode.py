...
class Episode:
    ...
    def segment(self, start: int, stop: int, should_pad: bool = False) -> Episode:
        ...
        def pad(x):
            pad_right   =   F.pad(x, [0 for _ in range(2 * x.ndim - 1)] + [padding_length_right]) if padding_length_right > 0 else x
            return F.pad(pad_right, [0 for _ in range(2 * x.ndim - 2)] + [padding_length_left, 0]) if padding_length_left > 0 else pad_right
