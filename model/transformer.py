...
from Transformer_World_Model.model.kv_caching import KeyValues, KVCache
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> torch.Tensor:
        ...
        q   =   self.q(x).view(B, T, self.num_heads, C // self.num_heads).transpose(1,2)    #   [B, nh, T, C // nh]
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, past_keys_values: Optional[KeyValues] = None) -> torch.Tensor:
...
class Transformer(nn.Module):
    ...
    def generate_empty_keys_values(self, n: int, max_tokens: int) -> KeyValues:
        device  =   self.ln_f.weight.device
        return KeyValues(n, self.config.num_heads, max_tokens, self.config.embed_dim, self.config.num_layers, device)

    def forward(self, sequences: torch.Tensor, past_keys_values: Optional[KeyValues] = None) -> torch.Tensor:
