...
class KeyValues:
    def __init__(self, n: int, num_heads: int, max_tokens: int, embed_dim: int, num_layers: int, device: torch.device) -> None:
        self._key_values    =   tuple([KVCache(n, num_heads, max_tokens, embed_dim, device) for _ in range(num_layers)])
    ...
    @property
    def size(self):
        return self._key_values[0].shape[2]
    
    def reset(self) -> None:
        for kv_cache in self._key_values:
            kv_cache.reset()
