...
from Transformer_World_Model.utils                      import compute_lambda_returns, LossWithIntermediateLosses
from Transformer_World_Model.model.tokenizer.tokenizer  import Tokenizer
from Transformer_World_Model.model.world_model          import WorldModel
from Transformer_World_Model.env.world_model_env        import WorldModelEnv
from Transformer_World_Model.dataset                    import Batch
...
class ActorCritic(nn.Module):
    def __init__(self, act_vocab_size: int, use_original_obs: bool = False) -> None:
        super().__init__()
        ...
        self.lstm_dim           =   512
        self.lstm               =   nn.LSTMCell(input_size=1024, hidden_size=self.lstm_dim)
    ...
    def reset(self, n: int, burnin_observations: Optional[torch.Tensor] = None, mask_padding: Optional[torch.Tensor] = None) -> None:
        ...
        if burnin_observations is not None:
            assert  burnin_observations.ndim == 5 and \
                    burnin_observations.size(0) == n and \
                    mask_padding is not None and \
                    burnin_observations.shape[:2] == mask_padding.shape
    ...
    def imagine(
        ...
        assert initial_observations.ndim == 5 and initial_observations.shape[2:] == (3, 64, 64)     #   [B, T, C, H, W]
        assert mask_padding[:, -1].all()

        device              =   initial_observations.device
        wm_env              =   WorldModelEnv(tokenizer=tokenizer, world_model=world_model, device=device)
        ...
        burnin_observations =   torch.clamp(
            ...
            0, 1
        )   if initial_observations.size(1) > 1 else None
